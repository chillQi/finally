...
txt1 = open("part2 共青团中央.json", "r", encoding='utf-8').read()
words1 = jieba.lcut_for_search(txt1)  # 使用jieba中的搜索引擎模式对3份文本进行分词
txt2 = open("part2 人民日报.json", "r", encoding='utf-8').read()
words2 = jieba.lcut_for_search(txt2)
txt3 = open("part2 央视新闻.json", "r", encoding='utf-8').read()
...
lineCount = 0  # 总行数
for line in open("part2 共青团中央.json", "r", encoding='utf-8'):
    lineCount += 1  # 总行数加1
    for i in range(len(counts)):
        word, count = items[i]
        if word in line:
            lines[word] = lines.get(word, 0) + 1  # 遍历所有词语，每出现一次其对应的行数加1
for line in open("part2 央视新闻.json", "r", encoding='utf-8'):
    lineCount += 1
    for i in range(len(counts)):
        word, count = items[i]
        if word in line:
            lines[word] = lines.get(word, 0) + 1
for line in open("part2 人民日报.json", "r", encoding='utf-8'):
    lineCount += 1
    for i in range(len(counts)):
        word, count = items[i]
        if word in line:
